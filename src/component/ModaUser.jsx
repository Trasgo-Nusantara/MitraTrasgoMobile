...
import { TouchableOpacity, Text, StyleSheet, Dimensions, View, Modal, Image, Linking } from 'react-native';
...
const ModalUser = ({ title, desc, isVisible, setModalVisible, actions, call, chat, selesai, data, status, driverLocation }) => {
  ...
  const openGoogleMaps = (latPick, lonPick, latDes, lonDes) => {
    const url = `https://www.google.com/maps/dir/${latPick},${lonPick}/${latDes},${lonDes}`;
    ...
    <View style={styles.modalContainer}>
      ...
      >
        <View style={styles.modalComponent} >
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            <View style={{ flex: 1, flexDirection: 'row-reverse' }}>
              <TouchableOpacity onPress={() => openGoogleMaps(
                ...
                data.data.destinationLocation.latitude,
                data.data.destinationLocation.longitude
              )} style={{ padding: 20, borderRadius: 100, backgroundColor: 'white', elevation: 1 }}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            <Text style={[COMPONENT_STYLES.textSmall, { fontWeight: '600' }]}>Penumpang</Text>
            <Text style={[COMPONENT_STYLES.textSmall, { fontWeight: '600' }]}>{data?.user?.fullName}</Text>
          </View>
          <View style={COMPONENT_STYLES.spacer} />
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            ...
            {status === 2 &&
              <Text style={[COMPONENT_STYLES.textSmall, { fontWeight: '600' }]}>Sedang mengantar penumpang</Text>
            }
            {status === 3 &&
          ...
          <View style={COMPONENT_STYLES.spacer} />
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            <Text style={[COMPONENT_STYLES.textSmall, { fontWeight: '600' }]}>Potongan Admin</Text>
            <Text style={[COMPONENT_STYLES.textSmall, { fontWeight: '600' }]}>Rp {data?.data?.hargaPotonganDriver.toLocaleString('id')}</Text>
          </View>
          <View style={COMPONENT_STYLES.spacer} />
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          ...
          {status === 1 &&
            <View style={{ justifyContent: 'space-around' }}>
              <View style={{ flex: 1 }}>
                <ButtonSlideComponent title={"Sudah Di Lokasi Tujuan"} onSlideComplete={()=> actions("a")}/>
              </View>
              <View style={COMPONENT_STYLES.spacer} />
              <View style={{ flex: 1 }}>
          ...
          {status === 2 &&
            <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
              <View style={{ flex: 1 }}>
              <ButtonSlideComponent title={"Selesai"} onSlideComplete={()=> actions("c")}/>
              </View>
            </View>
          }
        </View>
